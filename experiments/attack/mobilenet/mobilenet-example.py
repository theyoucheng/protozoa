...
### set up shap
#background = xs[np.random.choice(xs.shape[0], 3, replace=False)]
##background=np.ones((1,224,224,3))*255
#e = shap.DeepExplainer(model, background) ## OOM risk
...
eobj.testgen_factor=.2 #np.random.uniform(0,1) #0.05 #*xs[0].shape[2]
...
for idx in range(0, len(eobj.inputs)):
  ...
  for measure in ['zoltar', 'random', 'wong-ii', 'ochiai', 'tarantula']:
    ...
    partial_x, p_count, p_conf=to_attack(eobj, ranking_i, x, bg_v, 10, 10) #(32*32)//10)
    print ('++', eobj.measure, p_count/(img_rows*img_cols), p_conf)

    ofname='{0}/{1}.txt'.format(eobj.outputs, eobj.measure)
    f = open(ofname,"a") 
    f.write ('{0} {1}\n'.format(p_count, p_count/(img_rows*img_cols), p_conf))           
    f.close()
    #if measure=='tarantula' or measure=='zoltar':
  ...
  partial_x, p_count, p_conf=to_attack(eobj, ind, x, bg_v, 10, 10) #1) #(32*32)//10)
