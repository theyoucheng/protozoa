...
for idx in range(0, len(eobj.inputs)):
  ...
  st1=time.time()
  passing, failing=spectra_sym_gen(eobj, x, y[-1:], adv_value=eobj.adv_value, testgen_factor=eobj.testgen_factor, testgen_size=2000)
  et1=time.time()
  ...
  st2=time.time()
  passing2, failing2=spectra_sym_gen(eobj, x, y[-1:], adv_value=eobj.adv_value, testgen_factor=eobj.testgen_factor, testgen_size=200)
  et2=time.time()
  adv_xs200=[]
  adv_ys200=[]
  for e in passing2:
  ...
  for measure in ['zoltar', 'wong-ii', 'ochiai', 'tarantula']:
    ...
    ## testgen_size: 200
    selement200=sbfl_elementt(x, 0, adv_xs200, adv_ys200, model)
    st3b=time.time()
    ...
    f = open(ofname,"a") 
    f.write ('{0} {1} {2} {3} {4} {5}\n'.format(p_count, p_count200, et1-st1, et3a-st3a, et2-st2, et3b-st3b))           
    f.close()
    #if measure=='zoltar':
